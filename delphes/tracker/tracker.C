...
void tracker(std::string finname = "delphes.root",
	     std::string foutname = "tracker.root",
	     std::string what = "tracks")
{
  ...
  tin->SetBranchAddress(what.c_str(), &tracks);
  ...
  auto hBetaP = new TH2F("hBetaP", ";#it{p} (GeV/#it{c});v/#it{c}", 2000, 0., 20., 1000, 0.1, 1.1);
  TH2 *hGenEtaP[5], *hAccEtaP[5], *hHitEtaP[5], *hDeltaT[5][5], *hNsigma[5][5];
  for (Int_t ipart = 0; ipart < 5; ++ipart) {
    hGenEtaP[ipart] = new TH2F(Form("hGenEtaP_%s", pname[ipart]), Form("All generated (%s);#eta; #it{p} (GeV/#it{c})", plabel[ipart]), 200, -5., 5., 2000, 0., 10.);
    hAccEtaP[ipart] = new TH2F(Form("hAccEtaP_%s", pname[ipart]), Form("Generated in acceptance (%s);#eta; #it{p} (GeV/#it{c})", plabel[ipart]), 200, -5., 5., 2000, 0., 10.);
    hHitEtaP[ipart] = new TH2F(Form("hHitEtaP_%s", pname[ipart]), Form("Hitting the detector (%s);#eta; #it{p} (GeV/#it{c})", plabel[ipart]), 200, -5., 5., 2000, 0., 10.);
  ...
  for (Int_t iev = 0; iev < nevents; ++iev) {
    ...
    for (Int_t iparticle = 0; iparticle < nparticles; ++iparticle) {
      ...
      auto in_acceptance   = fabs(eta_true) < 0.5;
