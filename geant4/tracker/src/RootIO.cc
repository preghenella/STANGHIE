...
RootIO::AddTrack(const G4Track *aTrack) {
  mTracks.id[mTracks.n]     = aTrack->GetTrackID();
  mTracks.parent[mTracks.n] = aTrack->GetParentID();
  mTracks.pdg[mTracks.n]    = aTrack->GetParticleDefinition()->GetPDGEncoding();
  mTracks.e[mTracks.n]      = aTrack->GetKineticEnergy() / GeV;
  mTracks.px[mTracks.n]     = aTrack->GetMomentum().x()  / GeV;
  mTracks.py[mTracks.n]     = aTrack->GetMomentum().y()  / GeV;
  mTracks.pz[mTracks.n]     = aTrack->GetMomentum().z()  / GeV;
  mTracks.n++;
}
...
RootIO::AddHit(const G4Step *aStep)
{
  ...
  mHits.trkid[mHits.n]  = track->GetTrackID();
